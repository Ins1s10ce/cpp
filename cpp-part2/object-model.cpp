...
// 动态绑定: 调用在运行时决定函数地址
//   可以避免代码冗余 因为会大量使用if else语句等等
// 多态 <- 动态绑定 <- 虚机制

// C++ 调用函数时候 进行动态绑定的三个条件
// #1 通过指针或引用调用 
//      编译器不清楚指向对象的类型; 如果通过对象调用, 编译器知道类型, 静态调用(此时不需要虚机制)
// #2 指针up-cast,即子类指针转换为父类指针 
//      留到运行时决定, 方便调用不同子类函数; 否则编译器知道类型, 静态调用
// #3 调用虚函数 
//      多态: 同一个接口, 不同对象会调用不同的实现, 静态多态: 重载,模板, 动态多态: 继承+虚函数
int main() {
    B b;
